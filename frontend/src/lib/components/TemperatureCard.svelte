<script lang="ts">
	...
	// Default ranges
	const defaultMin = 0;
	const defaultMax = type === 'cpu' ? 90 : 80;

	// Reactive state for ranges
	let minTemp = $state(defaultMin);
	let maxTemp = $state(defaultMax);
	let isModalOpen = $state(false);

	// Load saved ranges from localStorage on mount
	onMount(() => {
		const saved = localStorage.getItem(`tempRange_${type}`);
		if (saved) {
	...
	const saveRange = (min: number, max: number) => {
		minTemp = min;
		maxTemp = max;
		localStorage.setItem(`tempRange_${type}`, JSON.stringify({ min, max }));
	};
